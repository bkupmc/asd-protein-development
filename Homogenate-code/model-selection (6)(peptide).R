...
repeat{
  # model to compare to
  system.time(re.null.BIC<-unlist(mclapply(model.peptides,lmerBICFunction,Y=log2(norm.pep[model.peptides,samples]),XX=meta.samples[samples,],
  ...
  for(covariate in setdiff(possible.covariates,null.model.covariates)){
    ...
    re.alt.BIC<-unlist(mclapply(model.peptides,lmerBICFunction,Y=log2(norm.pep[model.peptides,samples]),XX=meta.samples[samples,],
...
save(meta.samples,meta.pep,norm.pep,selected.model,re.model.selection,samples,pools,plexes,plex.border,plex.mid,pchDX,colPLEX,file=paste0("WorkData/MODEL-SELECTION-",tissue,".RData"))


### look for additional models including interactions

re.null.BIC=unlist(mclapply(rownames(norm.pep),lmerBICFunction,Y=log2(norm.pep[,samples]),XX=meta.samples[samples,],
                            covariates=selected.model,random.effect="PAIR",
                            mc.cores=64,mc.preschedule = T,mc.silent = T))
re.alt.BIC=unlist(mclapply(rownames(norm.pep),lmerBICFunction,Y=log2(norm.pep[,samples]),XX=meta.samples[samples,],
                           covariates=c(selected.model,"AGEyr:DX"),random.effect="PAIR",
                           mc.cores=64,mc.preschedule = T,mc.silent = T))
sum(re.alt.BIC - re.null.BIC < delta.cut["AGEyr"])  # 207


re.null.BIC=unlist(mclapply(rownames(norm.pep),lmBICFunction,Y=log2(norm.pep[,samples]),XX=meta.samples[samples,],
                           covariates=c(selected.model,"AGEyr:DX"),
                           mc.cores=16,mc.preschedule = T,mc.silent = T))
re.alt.BIC=unlist(mclapply(rownames(norm.pep),lmBICFunction,Y=log2(norm.pep[,samples]),XX=meta.samples[samples,],
                           covariates=c(selected.model,"AGEyr:DX","AGEyr2:DX"),
                           mc.cores=16,mc.preschedule = T,mc.silent = T))
sum(re.alt.BIC - re.null.BIC < delta.cut)  # 180

# no need to add interactions at this point
(selected.model=c(selected.model))

# neither meets the criteria of 0.05*nrow(norm.pep) (1644.1)
...
df$asd.id=rownames(meta.samples)[match(paste(df$pair,"ASD",sep="-"),paste(meta.samples$PAIR,meta.samples$DX,sep="-"))]

df$ctl.int=Y[feature,df$ctl.id]
df$asd.int=Y[feature,df$asd.id]
df$delta=df$asd.int-df$ctl.int
df$plex=meta.samples[df$asd.id,"PLEX"]
aggregate(delta~plex,data=df,FUN="mean")
aggregate(Y[feature,]~XX$PLEX,FUN="mean")
