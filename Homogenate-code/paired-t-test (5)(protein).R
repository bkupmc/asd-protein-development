...
# run the base analysis for all proteins use 1e3 permutations
system.time(re.perm.all<-rbindlist(mclapply(rownames(norm.prt),permutationTest,n.perm=1e3,log2.int = log2(norm.prt),df=df,mc.cores=min(190,nrow(norm.prt)),mc.preschedule = T,mc.silent=T)))
...
proteins=re.perm.all$feature[re.perm.all$p.emperical < 0.05] # 687
system.time(re.perm.05<-rbindlist(mclapply(proteins,permutationTest,n.perm=1e4,log2.int = log2(norm.prt[proteins,]),df=df,mc.cores=min(190,length(proteins)),mc.preschedule = T,mc.silent=T)))
...
proteins=re.perm.05$feature[re.perm.05$p.emperical < 0.005] # 174
system.time(re.perm.005<-rbindlist(mclapply(proteins,permutationTest,n.perm=1e5,log2.int = log2(norm.prt[proteins,]),df=df,mc.cores=min(190,length(proteins)),mc.preschedule = T,mc.silent=T)))
...
proteins=re.perm.005$feature[re.perm.005$p.emperical < 0.0005] # 34
system.time(re.perm.0005<-rbindlist(mclapply(proteins,permutationTest,n.perm=1e6,log2.int = log2(norm.prt[proteins,]),df=df,mc.cores=min(190,length(proteins)),mc.preschedule = T,mc.silent=T)))
...
pdf(paste0("Plots/emperical-p-values-proteins-",tissue,".pdf"),height=8,width=12)
hist(re.perm.all$p.emperical,breaks=seq(0,1,.01),las=1,xlab="emperical P",main="",col=viridis(5)[2])
dev.off()


1-qvalue(re.perm.all$p.emperical)$pi0 # 0.343
re.perm.all$q=qvalue(re.perm.all$p.emperical)$qvalues
sum(re.perm.all$q < 0.05)  # 67
sum(re.perm.all$q < 0.10)  # 224
sum(re.perm.all$q < 0.20)  # 605
max(re.perm.all$p.emperical[re.perm.all$q < 0.05]) #  .00107
max(re.perm.all$p.emperical[re.perm.all$q < 0.10]) # 0.0072
max(re.perm.all$p.emperical[re.perm.all$q < 0.20]) # 0.0387

colnames(re.perm.all)[1]="protein"
...
proteins=rownames(re.perm.all) # 4727
library(biomaRt)
mart = useMart("ensembl",dataset = "hsapiens_gene_ensembl")

reBM=getBM(c("uniprotswissprot","ensembl_gene_id","external_gene_name","chromosome_name"),filters="uniprotswissprot",
           values=proteins,mart=mart) # 5148
reBM=reBM[reBM$chromosome_name %in% c(1:24,"X","Y"),]  # 4729

length(unique(reBM$uniprotswissprot))  # 4704
length(unique(reBM$ensembl_gene_id))    # 4728
length(unique(reBM$external_gene_name)) # 4727
