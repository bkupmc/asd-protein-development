...
system.time(RESage <- mclapply(rownames(norm.prt),lmerResiduals,Y=log2(norm.prt[,samples]),XX=meta.samples[samples,],
                                covariates=selected.model,random.effect="PAIR",add.back=c("AGEyr","DXASD:AGEyr"),mc.cores=64,mc.preschedule=T))
...
pdf("SYNAPTOSOME-PROTEIN-AGE-mixture-modeling_11-23-2022.pdf",height=8,width=8)
...
WriteXLS::WriteXLS(RE,"SYNAPTOSOME-PROTEIN-AGE-mixture-modeling_11-23-2022.xlsx")
...
load("WorkData/TRAJECTORY-SYNAPTOSOME.RData")
...
plot(prt.clusters[prt.clusters$cluster == 2,"b.ctl"],prt.clusters[prt.clusters$cluster == 2,"b.asd"],xlim=c(-.08,0.05),ylim=c(-.08,0.05))
...
pdf("SYNAPTOSOMES-AGE-TRAJ-ASD-NT-2022-11-29.pdf",height=8,width=8)
plot(NULL,NULL,xlim=c(4,32),ylim=c(-.5,.55),las=1,xlab="Age",ylab="Effect")
...
sel.clusters=prt.clusters[prt.clusters$cluster == 2,]
sel.clusters[which(sel.clusters[,"b.asd"]-sel.clusters[,"b.ctl"]> 0.06),]

plot(prt.clusters[prt.clusters$cluster == 3,"b.ctl"],prt.clusters[prt.clusters$cluster == 3,"b.asd"])
abline(a=0,b=1)

t.test(prt.clusters[prt.clusters$cluster == 1,"b.ctl"],prt.clusters[prt.clusters$cluster == 1,"b.asd"],paired = T)

hist(prt.clusters[prt.clusters$cluster == 3,"b.asd"]-prt.clusters[prt.clusters$cluster == 3,"b.ctl"])

MLM=as.data.frame(read_excel("Results/SYNAPTOSOME-PROTEIN.xlsx",sheet="MLM.DX"))
...
WriteXLS::WriteXLS(prt.clusters,paste0("SYNAPTOSOMES-AGE-TRAJ-ASD-NT-",Sys.Date(),".xls"))
...
### compare homogenate and synaptosome
syn=as.data.frame(read_excel("SYNAPTOSOMES-AGE-TRAJ-ASD-NT-2022-11-29.xls"))
hom=as.data.frame(read_excel("../Homogenate-Pipeline-09-07-2022/HOMOGENATE-AGE-TRAJ-ASD-NT-2022-11-29.xls"))

hs=data.frame(protein=unique(c(syn$protein,hom$protein)))
rownames(hs)=hs$protein
hs[hom$protein,"chom"]=hom$cluster
hs[syn$protein,"csyn"]=syn$cluster

table(hs$chom,hs$csyn)
